...
- Each **opaque** glyph pixel (alpha > 128) becomes one filled 1×1 square in the outline; the font
  is monochrome, so CSS `color` tints it. `image-rendering` is irrelevant — it's real vector now.
